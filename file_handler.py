...
class BudgetFileHandler:
    ...
    def _resolve_safe_path(self, filename: str) -> str:
        """Resolve a relative filename and ensure it remains under base_path."""
        if os.path.isabs(filename):
            raise ValueError("Absolute paths are not allowed")

        base_path = os.path.realpath(os.path.abspath(self.base_path))
        filepath = os.path.realpath(os.path.abspath(os.path.join(base_path, filename)))

        try:
            is_within_base = os.path.commonpath([base_path, filepath]) == base_path
        except ValueError:
